...
class zibalHelperzibal
{
    ...
    public function __construct($method, $zibalPlugin)
    {
        ...
        $this->getzibalPaymentCurrency();
    }

    public function getzibalPaymentCurrency($getCurrency = false)
    {
    ...
    {
        $this->customerData = new zibalHelperCustomerData();
    ...
    {
        $this->debugLog($this->requestData, 'zibal '.$this->requestData['METHOD'].' Request variables ', 'debug');
    ...
    protected function sendRequest($post_data)
    {
        ...
        $this->debugLog($post_data, 'zibal '.$post_data['METHOD'].' Request variables:', $level);
        $this->debugLog($this->response, 'zibal response:', $level);
    ...
    public function getSSLCertificate(&$certifPath, &$passPhrase)
    {
        ...
        if ($safePath) {
            $sslCertifFolder = $safePath.'zibal';
        }
    ...
    protected function _getzibalUrl($protocol = 'https://', $includePath = true)
    {
        $url = ($this->_method->sandbox) ? $protocol.'www.sandbox.zibal.com' : $protocol.'www.zibal.com';
        if ($includePath) {
            $url .= '/cgi-bin/webscr';
        }
    ...
    protected function _getApiUrl()
    {
        ...
        //return ($this->_method->sandbox=='sandbox') ? 'https://api-3t.sandbox.zibal.com/nvp' : 'https://api-3t.zibal.com/nvp';
        return 'https://api'.$url_auth.$url_environment.'.zibal.com/nvp';
    }
    ...
    public function displayExtraPluginInfo()
    {
        ...
        if ($this->_method->payment_type == '_xclick-auto-billing' && $this->customerData->getVar('autobilling_max_amount')) {
            ...
            $extraInfo .= '<br/>';
            $extraInfo .= vmText::_('VMPAYMENT_zibal_PAYMENT_BILLING_MAX_AMOUNT').': '.$cd->priceDisplay($this->customerData->getVar('autobilling_max_amount'));
        }
...
//			$recurringDesc .= vmText::_('VMPAYMENT_zibal_TRIAL_PERIODS') . $this->_method->trial1_duration . ': '.$this->_method->trial1_price.'<br />';
//		}
//		$recurringDesc .= vmText::_('VMPAYMENT_zibal_SUBSCRIPTION_DURATION').': '.$this->_method->subscription_duration . '<br />';
//		$recurringDesc .= vmText::_('VMPAYMENT_zibal_SUBSCRIPTION_TERM').': '.$this->_method->subscription_term . '<br />';
        ...
        $recurringDesc = vmText::sprintf('VMPAYMENT_zibal_SUBSCRIPTION_DESCRIPTION', $durationValue, $durationUnit, $this->_method->subscription_term);
...
//		$recurringDesc .= vmText::_('VMPAYMENT_zibal_SUBSCRIPTION_TERM').': '.$this->_method->subscription_term . '<br />';
        ...
        $recurringDesc = vmText::sprintf('VMPAYMENT_zibal_PAYMENT_PLAN_DESCRIPTION', $this->_method->payment_plan_term, $durationValue, $durationUnit);
        if ($this->_method->payment_plan_defer && $this->_method->zibalproduct == 'std') {
            ...
            $recurringDesc .= '<br/>'.vmText::sprintf('VMPAYMENT_zibal_PAYMENT_PLAN_INITIAL_PAYMENT', JHTML::_('date', $startDate->toFormat(), vmText::_('DATE_FORMAT_LC4')));
        } else {
            if ($this->_method->payment_plan_defer_strtotime) {
                $startDate = JFactory::getDate($this->_method->payment_plan_defer_strtotime);
                $recurringDesc .= '<br/>'.vmText::sprintf('VMPAYMENT_zibal_PAYMENT_PLAN_INITIAL_PAYMENT', JHTML::_('date', $startDate->toFormat(), vmText::_('DATE_FORMAT_LC4')));
                //$recurringDesc .= '<br/>'.vmText::sprintf('VMPAYMENT_zibal_PAYMENT_PLAN_INITIAL_PAYMENT',date(vmText::_('DATE_FORMAT_LC4'),strtotime('first day of next month')));
            }
        }
    ...
    /********************************/
    public function processIPN($zibal_data, $payments)
    {

        // check that the remote IP is from zibal.
        if (!$this->checkzibalIps($zibal_data)) {
            return false;
        }
        // Validate the IPN content upon zibal
        if (!$this->validateIpnContent($zibal_data)) {
            return false;
        }
        //Check the zibal response
        /*
         * https://cms.zibal.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_html_IPNandPDTVariables
        ...
        if ($zibal_data['txn_type'] == 'subscr_cancel') {
            $order_history['order_status'] = $this->_method->status_canceled;
        } elseif ($zibal_data['txn_type'] == 'mp_cancel') {
            $order_history['order_status'] = $this->_method->status_canceled;
        } elseif ($zibal_data['txn_type'] == 'subscr_eot') {
            $order_history['order_status'] = $this->_method->status_expired;
        } elseif ($zibal_data['txn_type'] == 'recurring_payment_expired') {
            $order_history['order_status'] = $this->_method->status_expired;
        } elseif ($zibal_data['txn_type'] == 'subscr_signup') {
            //TODO: Validate the response
            $order_history['order_status'] = $this->_method->status_success;
        } elseif ($zibal_data['txn_type'] == 'recurring_payment_profile_created') {
            if ($zibal_data['profile_status'] == 'Active') {
                $order_history['order_status'] = $this->_method->status_success;
            } else {
                $order_history['order_status'] = $this->_method->status_canceled;
            }
        } else {
            if (strcmp($zibal_data['payment_status'], 'Completed') == 0) {
                ...
                if ($this->_check_txn_id_already_processed($payments, $zibal_data['txn_id'])) {
                    $this->debugLog($zibal_data['txn_id'], '_check_txn_id_already_processed', 'debug');

                    return false;
                }
                // 3. check email and amount currency is correct
                if ($zibal_data['txn_type'] != 'recurring_payment' && !$this->_check_email_amount_currency($payments, $zibal_data)) {
                    return false;
                }
                // now we can process the payment
                if (strcmp($zibal_data['payment_status'], 'Authorization') == 0) {
                    $order_history['order_status'] = $this->_method->status_pending;
                } else {
                    $order_history['order_status'] = $this->_method->status_success;
                }
                $order_history['comments'] = vmText::sprintf('VMPAYMENT_zibal_PAYMENT_STATUS_CONFIRMED', $this->order['details']['BT']->order_number);
            } elseif (strcmp($zibal_data['payment_status'], 'Pending') == 0) {
                $lang = JFactory::getLanguage();
                $key = 'VMPAYMENT_zibal_PENDING_REASON_FE_'.strtoupper($zibal_data['pending_reason']);
                if (!$lang->hasKey($key)) {
                    $key = 'VMPAYMENT_zibal_PENDING_REASON_FE_DEFAULT';
                }
                $order_history['comments'] = vmText::sprintf('VMPAYMENT_zibal_PAYMENT_STATUS_PENDING', $this->order['details']['BT']->order_number).vmText::_($key);
                $order_history['order_status'] = $this->_method->status_pending;
            } elseif (strcmp($zibal_data['payment_status'], 'Refunded') == 0) {
                if ($this->_is_full_refund($payments, $zibal_data)) {
                    $order_history['comments'] = vmText::sprintf('VMPAYMENT_zibal_PAYMENT_STATUS_REFUNDED', $this->order['details']['BT']->order_number);
                    $order_history['order_status'] = $this->_method->status_refunded;
                } else {
                    $order_history['comments'] = vmText::sprintf('VMPAYMENT_zibal_PAYMENT_STATUS_PARTIAL_REFUNDED', $this->order['details']['BT']->order_number);
                    $order_history['order_status'] = isset($this->_method->status_partial_refunded) ? $this->_method->status_partial_refunded : 'R';
                }
            } elseif (isset($zibal_data['payment_status'])) {
                // voided
                $order_history['order_status'] = $this->_method->status_canceled;
            } else {
                /*
                * a notification was received that concerns one of the payment (since $zibal_data['invoice'] is found in our table),
                * but the IPN notification has no $zibal_data['payment_status']
                * We just log the info in the order, and do not change the status, do not notify the customer
                */
                $order_history['comments'] = vmText::_('VMPAYMENT_zibal_IPN_NOTIFICATION_RECEIVED');
    ...
    protected function checkzibalIps($zibal_data)
    {
        /*
                $test_ipn = (array_key_exists('test_ipn', $zibal_data)) ? $zibal_data['test_ipn'] : 0;
                if ($test_ipn == 1) {
                    return true;
                }
        */
        $order_number = $zibal_data['invoice'];

        // Get the list of IP addresses for www.zibal.com and notify.zibal.com

        if ($this->_method->sandbox) {
            $zibal_iplist = gethostbynamel('ipn.sandbox.zibal.com');
            $zibal_iplist = (array) $zibal_iplist;
            $this->debugLog($zibal_iplist, 'checkzibalIps SANDBOX', 'debug', false);
        } else {
            $zibal_iplist1 = gethostbynamel('www.zibal.com');
            $zibal_iplist2 = gethostbynamel('notify.zibal.com');
            $zibal_iplist3 = ['216.113.188.202', '216.113.188.203', '216.113.188.204', '66.211.170.66'];
            $zibal_iplist = array_merge($zibal_iplist1, $zibal_iplist2, $zibal_iplist3);
// http://forum.virtuemart.net/index.php?topic=115375.msg406664#msg406664
            ...
            //------------api-aa.zibal.com------------
            $zibal_iplist_api_aa = ['173.0.88.67', '173.0.88.99', '173.0.84.99', '173.0.84.67'];
            //'------------api-3t.zibal.com------------'
            $zibal_iplist_api_3t_aa = ['173.0.88.69', '173.0.88.101', '173.0.84.69', '173.0.84.101'];
            //------------api-aa-3t.zibal.com------------
            $zibal_iplist_api_aa_3t = ['173.0.88.68', '173.0.88.100', '173.0.84.68', '173.0.84.100'];
            //------------notify.zibal.com (IPN delivery)------------
            $zibal_iplist_notify = ['173.0.81.1', '173.0.81.33'];
            //'-----------reports.zibal.com-----------'
            $zibal_iplist_reports = [
                '66.211.168.93',
                '173.0.84.161',
                '173.0.84.198',
                '173.0.88.161',
            ...
            //'------------www.zibal.com------------'
            //'Starting September 12, 2012 www.zibal.com will start resolving to a dynamic list of IP addresses and as such should not be whitelisted.'
            ...
            $zibal_iplist = array_merge($zibal_iplist, // Added JH 2013-10-12
                $zibal_iplist_api, $zibal_iplist_api_aa, $zibal_iplist_api_3t_aa, $zibal_iplist_api_aa_3t, $zibal_iplist_notify, $zibal_iplist_ipnb// JH
            );

            $this->debugLog($zibal_iplist, 'checkzibalIps PRODUCTION', 'debug', false);
        }
        $IP = $this->getRemoteIPAddress();
        $this->debugLog($IP, 'checkzibalIps REMOTE ADDRESS', 'debug', false);

        //  test if the remote IP connected here is a valid IP address
        if (!in_array($IP, $zibal_iplist)) {
            $text = 'Error with REMOTE IP ADDRESS = '.$IP.".
                        The remote address of the script posting to this notify script does not match a valid zibal IP address\n
            These are the valid IP Addresses: ".implode(',', $zibal_iplist).'The Order ID received was: '.$order_number;
            $this->debugLog($text, 'checkzibalIps', 'error', false);
    ...
    protected function validateIpnContent($zibal_data)
    {
        $test_ipn = (array_key_exists('test_ipn', $zibal_data)) ? $zibal_data['test_ipn'] : 0;
        if ($test_ipn == 1) {
            //return true;
        ...
        $zibal_data = $_POST;
        // zibal wants to open the socket in SSL
        $port = 443;
        $zibal_url = $this->_getzibalURL('ssl://', false);
        $zibal_url_header = $this->_getzibalURL('', false);
        $protocol = 'ssl://';
        /*
         * Before we can trust the contents of the message, we must first verify that the message came from zibal.
...
// read the post from zibal system and add 'cmd'
        $post_msg = 'cmd=_notify-validate';
        if (function_exists('get_magic_quotes_gpc')) {
            $get_magic_quotes_exists = true;
        }
        foreach ($zibal_data as $key => $value) {
        ...
        $header .= 'Host: '.$zibal_url_header.':'.$port."\r\n";
        $header .= "Content-Type: application/x-www-form-urlencoded\r\n";
        $header .= 'Content-Length: '.strlen($post_msg)."\r\n";
        //$header .= "Accept: */*\r\n\r\n";
        $header .= "Connection: close\r\n\r\n";

        $fps = fsockopen($zibal_url, $port, $errno, $errstr, 30);
        $valid_ipn = false;
        if (!$fps) {
            $this->debugLog(vmText::sprintf('VMPAYMENT_zibal_ERROR_POSTING_IPN', $errstr, $errno), 'validateIpnContent', 'error', false);
        } else {
            ...
            if (!$valid_ipn) {
                if (strstr($res, 'INVALID')) {
                    $errorInfo = ['zibal_data' => $zibal_data, 'post_msg' => $post_msg, 'zibal_res' => $res];
                    $this->debugLog($errorInfo, vmText::_('VMPAYMENT_zibal_ERROR_IPN_VALIDATION'), 'error', false);
                } else {
                    $this->debugLog(vmText::_('VMPAYMENT_zibal_ERROR_IPN_VALIDATION').': NO ANSWER FROM zibal', 'validateIpnContent', 'error', false);
                }
            }
        }
    ...
    protected function _check_email_amount_currency($payments, $zibal_data)
    {
        ...
        * If the payment is sent to a non-primary email address on your zibal account,
        * the receiver_email is still your primary email.
        */

        if ($this->_method->zibalproduct == 'std') {
            if (strcasecmp($zibal_data['business'], $this->merchant_email) != 0) {
                $errorInfo = ['zibal_data' => $zibal_data, 'merchant_email' => $this->merchant_email];
        ...
        $result = false;
        if ($this->_method->zibalproduct == 'std' and $zibal_data['txn_type'] == 'cart') {
            if (abs($payments[0]->payment_order_total - $zibal_data['mc_gross'] < abs($zibal_data['mc_gross'] * 0.001)) and ($this->currency_code_3 == $zibal_data['mc_currency'])) {
                $result = true;
            }
        } else {
            if (($payments[0]->payment_order_total == $zibal_data['mc_gross']) and ($this->currency_code_3 == $zibal_data['mc_currency'])) {
                $result = true;
            }
        }
        if (!$result) {
            $errorInfo = [
                'zibal_data'         => $zibal_data,
    ...
    public static function getzibalCreditCards()
    {
        return [
            'Visa',
            'Mastercard',
    ...
    public function _is_full_refund($payment, $zibal_data)
    {
        if (($payment->payment_order_total == (-1 * $zibal_data['mc_gross']))) {
            return true;
        } else {
            return false;
        }
    ...
    public function onShowOrderBEPayment($data)
    {
        ...
        if (isset($data->ACK)) {
            if ($data->ACK == 'SuccessWithWarning' && $data->L_ERRORCODE0 == self::FMF_PENDED_ERROR_CODE && $data->PAYMENTSTATUS == 'Pending'
                ...
                $html .= $this->zibalPlugin->getHtmlRowBE($prefix.$showOrderField, $this->highlight($data->$showOrderField));
            }
            if (($data->ACK == 'Failure' or $data->ACK == 'FailureWithWarning')) {
                $showOrderField = 'L_SHORTMESSAGE0';
                $html .= $this->zibalPlugin->getHtmlRowBE($prefix.'ERRORMSG', $this->highlight($data->$showOrderField));
        ...
        foreach ($showOrderBEFields as $key => $showOrderBEField) {
            if (($showOrderBEField == 'PAYMENTINFO_0_REASONCODE' and isset($data->$showOrderBEField) and $data->$showOrderBEField != 'None') or
                ...
                if (isset($data->$showOrderBEField)) {
                    $key = $prefix.$key;
                    $html .= $this->zibalPlugin->getHtmlRowBE($key, $data->$showOrderBEField);
                }
            }
        }
    ...
    public function debugLog($message, $title = '', $type = 'message', $echo = false, $doVmDebug = false)
    {
        ...
        $this->zibalPlugin->debugLog($message, $title, $type, $doVmDebug);
    }
}
